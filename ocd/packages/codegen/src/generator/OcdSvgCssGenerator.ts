...
export class OcdSvgCssGenerator extends OcdCodeGenerator {
    ...
    writeFiles(outputDirectory: string, resource: string, force = false) {
        ...
        // Embed each CSS file as a String.raw template literal. String.raw only
        // disables backslash escapes; backticks and ${ ... } sequences would still
        // terminate / interpolate the literal, so they must be escaped to keep the
        // generated TypeScript valid for any CSS content (e.g. comments that include
        // `className={`...${theme}...`}`).
        const escapeTemplate = (css: string): string => css.replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\$\{/g, '\\${')
...
export const svgCssData: Record<string, string> = {
    ${cssFiles.map((f) => `'${f}': \`${escapeTemplate(fs.readFileSync(path.join(resource, f), 'utf8'))}\``).join(',\n\n    ')}
}
